...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while not test_place.is_hive:
            if distance >= self.lower_bound and distance <= self.upper_bound:
                bee = random_bee(test_place.bees)
                if bee:
                    return bee
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if current_place:
            copy_bees_list = list(current_place.bees)
            for b in copy_bees_list:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        double_damage_place = self.place.exit
        while double_damage_place:
            cur_place_ant = double_damage_place.ant
            if cur_place_ant:
                cur_place_ant.double()
                if cur_place_ant.is_container and cur_place_ant.ant_contained:
                    cur_place_ant.ant_contained.double()
            double_damage_place = double_damage_place.exit
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return self.damage - (0.0625 * self.insects_shot) - (distance * 0.25)
        # END Problem Optional 2
