...
def about(subject):
    ...
    def contain(paragraph):
        for word in lower(remove_punctuation(paragraph)).split():
            if word in subject:
                return True
        return False
    return contain
...
def feline_fixes(typed, source, limit):
    ...
    def compare(t, s, count):
        if t == "" and s == "":
            return count
        if t == "" or s == "":
            return count + abs(len(t) - len(s))
        if count > limit:
            return count
        if t[0] == s[0]:
            return compare(t[1:], s[1:], count)
        return compare(t[1:], s[1:], count + 1)
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            ...
            if cur_time < min_time:
                min_time = cur_time
                min_player = j
        result[min_player] += [get_word(match, i)]
    return result
    # END PROBLEM 10
