...
def about(subject):
    ...
    def check(paragraph):
        for s in subject:
            for p in split(remove_punctuation(paragraph)):
                if s == lower(p):
                    return True
        return False
    return check
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for w in word_list:
        ...
        if diff < min_diff or (diff == limit and word == typed_word):
            min_diff = diff
            word = w
...
def minimum_mewtations(typed, source, limit):
    ...
    def count(t, s, n):
        if n > limit:
            return n
        if t == '' or s == '':
            return n + len(s) + len(t)
        if t[0] == s[0]:
            return count(t[1:], s[1:], n)
        else:
            add = count(t, s[1:], n + 1)
            remove = count(t[1:], s, n + 1)
            substitute = count(t[1:], s[1:], n + 1)
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = [[]]
    for player_time in timestamps_per_player:
        cur = player_time[0]
        time = []
        for t in player_time[1:]:
            time += [t - cur]
            cur = t
        times += [time]
    return match(words, times[1:])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        result[min_index] += [get_word(match, i)]
    return result
    # END PROBLEM 10
