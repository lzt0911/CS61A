...
class Account:
    ...
    def deposit(self, amount):
        ...
        after = self.balance
        t = Transaction(self.id, before, after)
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            t = Transaction(self.id, before, before)
            self.transactions.append(t)
            self.id += 1
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if coin in self.coins:
            ...
            self.coins[coin] = 1
        copy_coins = dict(self.coins)
        result = make_change(coin, copy_coins)
        if result:
            for c in result:
