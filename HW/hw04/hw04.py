...
def balanced(m):
    ...
    if is_planet(m):
        return True
    left_mass = length(left(m)) * total_mass(end(left(m)))
    right_mass = length(right(m)) * total_mass(end(right(m)))
...
def max_path_sum(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    def calculate_path_sum(t, cur):
        if is_leaf(t):
            return cur + label(t)
        return max([calculate_path_sum(b, cur + label(t)) for b in branches(t)])
    return calculate_path_sum(t, 0)
